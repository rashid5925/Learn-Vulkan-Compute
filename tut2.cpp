...
int bufferSize = (N + N + (N * N)) * sizeof(float);
...
int main() {    
    ...
    descriptorBufferInfo1.range = N * sizeof(float);
    descriptorBufferInfo1.offset = N * sizeof(float) * 0;

    VkDescriptorBufferInfo descriptorBufferInfo2;
    descriptorBufferInfo2.buffer = buffer;
    descriptorBufferInfo2.range = N * sizeof(float);
    descriptorBufferInfo2.offset = N * sizeof(float) * 1;
    ...
    descriptorBufferInfo3.offset = N * sizeof(float) * 2;
    ...
    memcpy(mappedData, dataA.data(), N * sizeof(float)); 
    memcpy((char*)mappedData + N * sizeof(float), dataB.data(), N * sizeof(float));
    vkUnmapMemory(device, memory);

    auto computeShaderCode = readSPV("arr-mul.spv");
